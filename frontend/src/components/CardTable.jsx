...
const CardTable = ({ cards, onViewCard, isLoading }) => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12 md:py-20">
        <div className="w-6 h-6 md:w-8 md:h-8 border-2 border-neon-cyan border-t-transparent rounded-full animate-spin" />
        <span className="ml-3 md:ml-4 text-neon-cyan font-mono text-sm md:text-base">Carregando dados...</span>
      </div>
    );
  }
  ...
  if (!cards || cards.length === 0) {
    return (
      <div className="text-center py-12 md:py-20" data-testid="empty-state">
        <CardIcon className="w-12 h-12 md:w-16 md:h-16 text-white/20 mx-auto mb-4" />
        <p className="text-white/50 font-mono text-sm md:text-base">Nenhum cartão encontrado</p>
        <p className="text-white/30 text-xs md:text-sm mt-2">Os cartões aparecerão aqui quando disponíveis</p>
      </div>
    );
  }
  ...
  const MobileCardItem = ({ card }) => (
    <div 
      className="bg-cyber-gray border-l-2 border-transparent hover:border-neon-cyan transition-all duration-300 rounded-lg p-4 mb-3"
    ...
    >
      ...
      <button
        ...
        className="btn-cyber w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-neon-cyan text-neon-cyan font-orbitron text-xs uppercase tracking-widest hover:bg-neon-cyan hover:text-black transition-all duration-300 rounded"
